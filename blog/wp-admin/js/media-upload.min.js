...
//    if (b && !b.isHidden()) {
    if (b) {
        ...
        if( id('add-post-image') != null &&  id('add-post-image').className.indexOf('hanging') > -1 ){
            getImgFromWare( c );
        }else{
            b.execCommand("mceInsertContent", false, c);
        }
        var id = function( idString ){
            return document.getElementById( idString );
        };
        /*
        var getRelativePath = function(){
            var url = arguments[0];
            var pathArr = url.split('/blog/wp-content/uploads/');
            return pathArr[1];
        };

        if( id('add-post-image').className.indexOf('hanging') > -1 ){
            var clsName = id('add-post-image').className;
            clsName = clsName.replace(/(\s+)?(hanging)(\s+)?/g,'');
            id('add-post-image').className = clsName;
            id('post-image-frm').innerHTML = c;

            var imgDom = id('post-image-frm').getElementsByTagName('IMG')[0];
            id('post-image-frm').innerHTML = '';
            id('post-image-frm').appendChild(imgDom);
            var path = imgDom.src;
            var path = getRelativePath( path );
            id('post-image-path').value =  path;
        }else{
            b.execCommand("mceInsertContent", false, c)
        }*/
